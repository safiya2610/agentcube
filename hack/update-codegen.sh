...
export PATH="/c/Program Files/Go/bin:/c/Users/safiy/go/bin:$PATH"
...
# Ensure codegen binaries are installed in GOPATH/bin
...
sed -i 's/path\.Base(\(.*\))/path.Base(filepath.ToSlash(\1))/g' "${TMP_CODEGEN}/cmd/informer-gen/generators/targets.go"
